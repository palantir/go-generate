...
`,
	...
	_, err = gofiles.Write(tmpDir, specs)
	require.NoError(t, err)
...
`,
		...
		outBuf := &bytes.Buffer{}
		verifyOK, err := gogenerate.Verify(currCaseDir, cfg.ToParam(), outBuf)
		require.NoError(t, err, fmt.Sprintf("Case %d: %s", currCaseNum, currCase.name))
		require.False(t, verifyOK, fmt.Sprintf("Case %d: %s", currCaseNum, currCase.name))

		assert.Equal(t, currCase.wantOutput, outBuf.String(), "Case %d: %s", currCaseNum, currCase.name)
	}
}
